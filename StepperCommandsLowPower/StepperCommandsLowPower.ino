...
/*
  ...
  20180919
  Added lowering al pin after a command
...
int16_t calculateDifferenceBetweenSteps(int16_t startPosition, int16_t endPosition)
{
  ...
  while (difference < -HalfMaxPass) difference += TurnPasses;   //????? why not if?????
...
void setup() {
  ...
  DEBUG.println(F("StepperCommandsLowPower"));
